...
the hell out of me and making me sure to \n get out once in awhile. My favorite scene is when he is talking to a ghost
...
texts_corpus_dict = {'nick': nick, 'paul': paul, 'kate': kate, 'jane':jane}

def preprocess_texts(texts_corpus_dict):

    '''Funcion for preprocessing strings: making lower, deleting punctuation, tokenizing (splitting)'''

    import string

    preprocessed_texts_corpus = {}

    # operating on each key[value] pairs for every text
    for key, value in texts_corpus_dict.items():
        # deleting punctuation
        value = value.translate(str.maketrans('', '', string.punctuation))
        # tokenizing and lowering
        value = value.lower().split()
        # getting lengths of unique set of words
        value = int(len(set(value)))
        # writing authors (key) and lengths (value) of the processed texts to the new dictionary
        preprocessed_texts_corpus[key] = value

    return preprocessed_texts_corpus


def best_writer_sorting(preprocessed_corpus):

    ''' function for making dictionary sorting and fetching the highest value and key'''

    count = 0
    name = 0

    for key, value in preprocessed_corpus.items():
        # simple sorting, saving the highest value
        if value > count:

            count = value
            name = key

    return print(name, count)


preprocessed_corpus = preprocess_texts(texts_corpus_dict)
best_writer_sorting(preprocessed_corpus)
