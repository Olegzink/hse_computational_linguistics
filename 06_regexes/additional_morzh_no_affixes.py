...
##################
# #### Version 2: substitute only form: 'морж', 'Морж' without affixes
##################
...
text_no_entry_forms = text

pattern_single_entry_sm = re.compile(r'морж\s')
pattern_single_entry_cap = re.compile(r'Морж\s')
text_no_entry_forms = re.sub(pattern_single_entry_sm, 'корж ', text_no_entry_forms)
text_no_entry_forms = re.sub(pattern_single_entry_cap, 'Корж ', text_no_entry_forms)

# working with entry of type 'Морж[1]'
result = re.findall(r'Морж\[[0-9]+\]', text_no_entry_forms, re.MULTILINE)
for x in result:
    if x in text_no_entry_forms:
        text_no_entry_forms = re.sub(r'Морж', 'Корж', text_no_entry_forms)

print(text_no_entry_forms)
